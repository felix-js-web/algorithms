...
import static sprint4.YandexInsertionSort.insertionSortToTheRightT;

class YandexInsertionSortTest {

    @Test
    void insertionSortToTheRight() {
        Random random = new Random();
        // Генерируем и проверяем 1000 тестов
        for (int test = 0; test < 1000; test++) {
            // Генерация случайного массива длиной от 1 до 1000 с элементами от -1000 до 1000
            int arrayLength = random.nextInt(10) + 1; // Длина от 1 до 1000
            int[] randomArray = new int[arrayLength];

            for (int i = 0; i < arrayLength; i++) {
                randomArray[i] = random.nextInt(2001); // Значения от -1000 до 1000
            }
            ...
            // Сортируем массивы
            sortedBySelectionSort = insertionSortToTheRightT(sortedBySelectionSort);
